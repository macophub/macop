...
const (
	mediaTypeContainer = "application/vnd.macop.container.v1"
...
type ContainerInfo struct {
	Copy         []MacopConfigSpecFileCopy `json:"copy,omitempty"`
	BinFilepath  string                    `json:"bin_filepath,omitempty"`
	Architecture string                    `json:"architecture"` // E.g. amd64, ppc64le
	OS           string                    `json:"os"`           // E.g. linux
}

func NewContainerLayer(info ContainerInfo) (*Layer, error) {
	var b bytes.Buffer
	if err := json.NewEncoder(&b).Encode(info); err != nil {
		return nil, err
	}
	layer, err := NewLayer(&b, "application/vnd.macop.container.v1")
	if err != nil {
		return nil, err
	}
	layer.Platform.OS = info.OS
	layer.Platform.Architecture = info.Architecture
	return &layer, nil
}
